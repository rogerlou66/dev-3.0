...
decides (`-s`/`--sandbox`, `--dangerously-bypass-approvals-and-sandbox`, or a `-c` naming
`default_permissions` / `sandbox_mode` / `sandbox_permissions`). The same rule adds the dev3
