...
6. **Work across projects → use Dashboard → Board.** The workspace board aligns every project as a swimlane; drag the project handle to put important projects first (or use the up/down controls on narrow screens), while Operations stays pinned. Questions stay in Agent Working as amber **Needs input** cards, AI Review appears only when occupied, and PR Review stays visible as the external-review queue. Click an active task to float its full inspector + coding terminal over the board; click the dimmed Kanban around it, close it, or choose **Global Kanban** to return to the exact same board position, or choose **Open full page** when you want to leave the cross-project view. The compact cards keep titles to one line; hover one to read the complete title and latest agent update. Cancelled tasks remain available on each project's board instead of occupying the daily workspace matrix. Open **Projects** beside Board for project management.
