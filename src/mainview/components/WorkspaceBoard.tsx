	...
	return (
		...
		<div className="flex items-center gap-2 border-b border-edge px-3 py-2">
			<div className="relative max-w-sm flex-1">
				...
				<input aria-label={t("workspaceBoard.search")} value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("workspaceBoard.search")} className="w-full rounded-lg border border-edge bg-base/50 py-2 pl-9 pr-3 text-sm text-fg outline-none focus:border-accent" />
			</div>
			{loadErrors.length > 0 && <button onClick={load} className="rounded-lg border border-warning/40 bg-warning/10 px-3 py-2 text-xs text-warning">{t("workspaceBoard.retry")}</button>}
		</div>
		{isNarrow ? (
