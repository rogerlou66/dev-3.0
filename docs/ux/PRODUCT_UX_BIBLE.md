...
- **App type:** Multi-client terminal workspace. The computer runs the Bun authority and either the Electrobun desktop shell or `dev3 remote`; browsers and the Android tablet shell render the shared React 19 + Tailwind + Vite client. Not Electron, not a screen-streaming product.
...
| Task | `project` (card) | `task` (full terminal) / split in `project` / transient overlay over `dashboard` | project | create, move status, rename, set overview, note, spawn variants, add attempts, duplicate, delete, watch, open-in, git, dev-server | `types.ts (Task)`, `TaskCard.tsx`, `TaskInfoPanel.tsx`, `WorkspaceTaskOverlay.tsx` |
...
| Paired host (Android-local) | Android connection gateway | existing connected dev3 route tree | Android client | scan/paste connection link, reconnect, forget | `android/`, `android-client-bridge.ts` |
...
**Workspace Board (`Observed`, 2026-08-28):** Dashboard → Board fixes a horizontal Inbox of `user-questions` and `review-by-user` duplicates above the independently scrolling lifecycle grid; its left rail carries a counter and a counter-clockwise vertical label. Desktop cards use two title lines and left priority/stage controls (64px); Done discloses completed cards; 36px empty rows retain `New Task`; AI Review remains a separate occupied-only drop target. Same-project drag, task overlay/detail opening, priority/status controls, Custom, project navigation/reordering, search, live refresh, pinned Operations, and the narrow carousel remain intact; Cancelled stays on project boards.
...
**Blocked reviews (2026-09-02):** Both boards retain Blocked after Your Review. Existing drag/status menus park settled human/peer reviews without a required reason or extra button. The persistent hold excludes Inbox/Needs You while retaining status/workspace; hooks cannot clear it. Explicit unblock/drag out or To Do/terminal cleanup clears it. Sidebar: separate Blocked tier; search: `is:blocked`/`status:blocked`; wait time: separate. See `decisions/2026/09/02/blocked-review-hold.md`.
...
| Workspace task overlay | one focused `92vw × 88dvh` window over an inert-but-mounted board | **full-bleed task workspace**; close restores the exact board carousel/scroll state | `Observed` |
...
| Terminal text input (touch) | direct typing into the focused terminal | **docked composer** is an input-model switch, never a width switch. Browser touch uses `TerminalComposer`; the Android shell uses one native multiline dock for task and project terminals so IME composition and per-keystroke paint never enter the WebView. Task Send preserves `delivered / unconfirmed / not-delivered`; only confirmed delivery clears the draft. Project Send and Insert remain explicitly unconfirmed and keep the draft. Sticky `⌨` **raw** restores direct PTY input. | `Observed` |
...
- **Android is one client shell, not a second product IA.** The connection gateway exists only before a host is available; after connection it enters the same dashboard/project/task route tree. Platform-local actions use Android intents, host actions use RPC, and desktop-window chrome with no Android meaning is omitted with an explanation rather than exposed as a no-op.
